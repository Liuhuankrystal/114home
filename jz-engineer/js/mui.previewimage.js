(function($, window) {
	...
	proto.initEvent = function() {
		...
		$(document.body).on('tap', 'img[data-preview-src]', function() {
			//任务 1478: 对于评估机构app与pc端缩略图用L结尾路径的图片，以节省流量
			plus.nativeUI.showWaiting("加载中...");
			var imgpic = document.getElementsByTagName("img");
			for(i=0;i<imgpic.length;i++){
				var imgurl = imgpic[i].src;
				imgurl = imgurl.replace("_L.jpg","_M.jpg");
				imgurl = imgurl.replace("_M.jpg","_H.jpg");
				imgpic[i].src = imgurl;
			}
			//任务 1478: 对于评估机构app与pc端缩略图用L结尾路径的图片，以节省流量end
